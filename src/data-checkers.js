...
export default {
  ...
  checkPassword(value) {
    const PASSWORD_REGEX = /^(?=.*[0-9])(?=.*[!@#$%^&*])[a-zA-Z0-9!@#$%^&*]{8,50}$/;
    return PASSWORD_REGEX.test(value);
  },
  checkCity(value) {
    const CITY_REGEX = /^(?:[A-Za-z]{2,}(?:(\.\s|'s\s|\s?-\s?|\s)?(?=[A-Za-z]+))){1,2}(?:[A-Za-z]+)?$/;
    return CITY_REGEX.test(value);
  },
  checkCountry(value) {
    const COUNTRY_REGEX = /^(?:[A-Za-z]{2,}(?:(\.\s|'s\s|\s?-\s?|\s)?(?=[A-Za-z]+))){1,2}(?:[A-Za-z]+)?$/;
